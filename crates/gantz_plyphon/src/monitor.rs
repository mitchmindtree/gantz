...
//! The node's control `expr` simply surfaces this state on a trigger push.

use gantz_core::node::state;
use gantz_core::steel::SteelVal;
use gantz_core::steel::steel_vm::engine::Engine;

/// Append `values` to the ring-buffer list at the node `path` in VM state,
/// dropping the oldest so the ring holds at most `size` samples (a `size` of 0 is
/// treated as 1 - the ring always keeps at least the latest sample).
///
/// The state is a [`SteelVal::ListV`] of numbers (seeded empty in the node's
/// `register`); a non-list or absent value is treated as an empty ring. Mirrors
/// `plot`'s scope-history push, but batches a whole frame's samples into one
/// state write.
pub fn push_ring(vm: &mut Engine, path: &[usize], values: &[f32], size: usize) {
    let size = size.max(1);
    let mut list = match state::extract_value(vm, path) {
        Ok(Some(SteelVal::ListV(list))) => list,
        _ => Default::default(),
    };
    for &v in values {
        list.push_back(SteelVal::NumV(v as f64));
    }
    while list.len() > size {
        list.pop_front();
    }
    let _ = state::update_value(vm, path, SteelVal::ListV(list));
}
